# GitHub Copilot Instructions for HuduCommunityBot
...
Never manually edit `src/HuduCommunityBot/HuduCommunityBot.csproj` version or `CHANGELOG.md` version entries.

Run the VersionManager workflow when changes affect shipped bot runtime behavior (for example, code under `src/HuduCommunityBot/`, runtime configuration behavior, commands, services, or user-visible functionality).
...
Build validation enforces version consistency. If `HuduCommunityBot.csproj` and `CHANGELOG.md` versions differ, build fails.
