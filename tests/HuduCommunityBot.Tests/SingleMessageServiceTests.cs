...
public sealed class SingleMessageServiceTests : IAsyncDisposable
{
    ...
    public SingleMessageServiceTests()
    {
        ...
        services.AddDbContext<HuduCommunityBotContext>(o =>
    ...
    private async Task<T> UseDbAsync<T>(Func<HuduCommunityBotContext, Task<T>> action)
    {
    ...
    private async Task UseDbAsync(Func<HuduCommunityBotContext, Task> action)
    {
        await using var scope = _scopeFactory.CreateAsyncScope();
        var db = scope.ServiceProvider.GetRequiredService<HuduCommunityBotContext>();
        await action(db);
