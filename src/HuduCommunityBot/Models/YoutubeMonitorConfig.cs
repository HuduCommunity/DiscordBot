...
public class YoutubeMonitorConfig
{
    ...
    public void Validate()
    {
        ...
        {
            if (ForumChannelId == 0)
                throw new InvalidOperationException("YoutubeMonitorConfig: When enabled, ForumChannelId must be a valid Discord forum channel ID (non-zero). Current value: 0. Check HUDUCOMMUNITYBOT_Bot__YoutubeMonitor__ForumChannelId environment variable. Must be a single channel ID, not 'guildId/channelId'.");

            if (string.IsNullOrWhiteSpace(YouTubeDataApiKey))
                throw new InvalidOperationException("YoutubeMonitorConfig: When enabled, YouTubeDataApiKey is required. Set HUDUCOMMUNITYBOT_Bot__YoutubeMonitor__YouTubeDataApiKey.");
